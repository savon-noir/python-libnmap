...
class NmapProcess:
    def __init__(self, targets="127.0.0.1",
                 options="-sT", event_callback=None):
    ...
    def run(self):
        ...
        self._run_init()
        try:
            _tmp_cmdline = shlex.split(self._nmap_command_line)
            self._nmap_proc = subprocess.Popen(args=_tmp_cmdline,
                                               stdin=subprocess.PIPE,
                                               stdout=subprocess.PIPE,
                                               stderr=subprocess.PIPE)
            tout = Thread(target=stream_reader, name='stdout-reader',
                          args=(self._nmap_proc.stdout, self.__io_queue))
            terr = Thread(target=stream_reader, name='stderr-reader',
                          args=(self._nmap_proc.stderr, self.__ioerr_queue))
            tout.start()
            terr.start()
            ...
            for e, xmlnode in edomdoc:
                if e is not None and e == pulldom.START_ELEMENT:
                    if (xmlnode.nodeName == 'taskprogress' and
                        ...
                        self.update_progress(percent_done, etc_done)
                        rval = True
